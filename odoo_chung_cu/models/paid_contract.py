...
class PaidContract(models.Model):
    _name = 'paid.contract'
    ...
    power_id = fields.One2many('quan.ly.chung.cu.dien', 'paid_contract_id', string='Điện')
    water_id = fields.One2many('quan.ly.chung.cu.nuoc', 'paid_contract_id', string='Nước')
    ...
    def default_get(self, fields_list):
        ...
        if cxt.get('active_model') == 'quan.ly.chung.cu.hop.dong':
            contract = self.env['quan.ly.chung.cu.hop.dong'].browse([cxt.get('active_id'),])
            defaults['room_id'] = contract.product_room.room_product.id
            defaults['contract_id'] = contract.id
            defaults['supplier_id'] = contract.partner_id.id
